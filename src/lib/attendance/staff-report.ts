...
async function fetchSchoolStaffRoles(
  ...
  // Fetch roles, profiles, and custom roles in parallel to reduce sequential database network roundtrip latency.
  const [rolesRes, profilesRes, customRolesRes] = await Promise.all([
    supabase
  ...
  if (rolesRes.error) {
    console.warn('[staff-report] roles:', rolesRes.error.message);
    return [];
  }
