...
export async function resolveReportCapabilities(
  ...
  // Handle staff role directly if no teacher or admin role is present
  const hasTeacherOrAdmin = session.roles.some((r) => ['school_admin', 'super_admin', 'teacher'].includes(r.role));
  const staffRole = session.roles.find((r) => r.role === 'staff');
  if (staffRole?.school_id && !hasTeacherOrAdmin) {
