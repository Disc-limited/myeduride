...
async function findSiblingLinkedParent(
  ...
  if (normalizedUsername) orConditions.push(`username.eq.${normalizedUsername}`);
  ...
  // Step 1: Find matching parent user profiles directly using OR filter
  ...
  // Step 2: Query student_parents to see if any matches exist under the same school
  // and are linked to an active student (excluding the current one)
  const { data: links, error: linkErr } = await supabase
    .from('student_parents')
    .select('parent_user_id, student:students!inner(id, school_id, is_active)')
  ...
  const matchingProfile = profiles.find((p) => p.id === firstLink.parent_user_id);
  if (matchingProfile) {
    return {
