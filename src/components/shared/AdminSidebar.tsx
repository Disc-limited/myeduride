...
import {
  ...
  ChevronDown, ChevronRight, Plus, List, Shield, KeyRound, UserCheck, User, MessageSquare
} from 'lucide-react';
...
const bottomLinks: NavLink[] = [
  { label: 'EduChart', href: '/dashboard/school-admin/messages', icon: <MessageSquare size={18} /> },
...
export function AdminSidebar() {
  ...
  const fetchUnreadTotal = async () => {
    try {
      ...
      console.error('Failed to fetch unread total in sidebar:', e);
    }
  };
  ...
  const renderLink = (item: NavLink, nested = false) => {
    ...
    const isEduChart = item.label === 'EduChart';
    return (
      <Link
        key={item.href}
        href={item.href}
      ...
      >
        ...
        {isEduChart && unreadEduChartCount > 0 && (
  ...
  const navContent = (
    <>
      <div className="p-5">
        <Link href="/dashboard/school-admin" className="flex items-center gap-2" onClick={() => setMobileOpen(false)}>
          <img src={LOGO_URL} alt="MyEduRide" className="h-10 w-auto object-contain" />
        </Link>
      </div>
