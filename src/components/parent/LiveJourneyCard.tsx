...
import { Navigation, Bus, ArrowRight, Layers, Plus, Minus, MapPin, Compass, ShieldCheck } from 'lucide-react';
...
export default function LiveJourneyCard({
  ...
  // Animate shuttle movement along route
  useEffect(() => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-xs flex flex-col justify-between h-full hover:shadow-md transition-all">
      ...
      <div className="relative my-3 rounded-2xl overflow-hidden border border-slate-200 shadow-inner h-[150px] bg-slate-100 group">
        ...
        >
          ...
          {/* Bus Badge Marker */}
          <div className="relative w-8 h-8 rounded-xl bg-amber-400 border-2 border-white shadow-lg flex items-center justify-center text-slate-900 font-bold">
        ...
        <div className="absolute right-2.5 bottom-7 z-30 flex flex-col bg-white/95 rounded-lg border border-slate-200/80 shadow-sm">
          <button
            ...
            className="p-1 text-slate-600 hover:bg-slate-100 border-b border-slate-100"
            title="Zoom In"
          >
        ...
        <div className="absolute right-2.5 top-2.5 bg-white/95 border border-slate-200/90 shadow-md backdrop-blur-md px-2.5 py-1 rounded-xl text-center z-30">
          <span className="text-sm font-black text-slate-900 block leading-none">{etaMinutes}</span>
        ...
        <div className="absolute left-2.5 bottom-1.5 z-30 flex items-center gap-1 select-none pointer-events-none">
          ...
          <span className="text-[8px] text-slate-500 bg-white/60 px-1 rounded hidden xs:inline">
            ©2026 Google
          </span>
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={onOpenLiveJourney}
        className="mt-3 w-full bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-all shadow-2xs hover:border-slate-300 active:scale-98"
      >
