...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Compile Student Cards Layout Metadata
    ...
    // 2. Compile Staff Cards Layout Metadata (Optimized Multi-Image Filter)
    if (staff_role_ids?.length) {
      ...
      for (const r of roles || []) {
        ...
        // Isolate the primary avatar photo to bypass face-api asset arrays
      ...
      } // <-- FIXED: Added the missing closing block brace for the staff loop structure
    }
    ...
    // 3. Render and Stream PDF Payload Binary
