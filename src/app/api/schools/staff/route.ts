...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: staffProfiles } = await supabase
      ...
      .select('id, user_id, staff_id_number, qr_code_data, photo_url, custom_role_id, teacher_responsibility')
      .eq('school_id', schoolId);

    const staffProfilesList: any[] = staffProfiles || [];

    const profileIds = staffProfilesList.map((p) => p.id).filter(Boolean);
    let classAssignments: any[] = [];
    if (profileIds.length > 0) {
      const { data: assignmentsData } = await supabase
        .from('teacher_class_assignments')
        .select('teacher_profile_id, class_id, class:school_classes(name)')
        .in('teacher_profile_id', profileIds);
      classAssignments = assignmentsData || [];
    }
    ...
    for (const r of roles || []) {
      ...
      const assignment = classAssignments.find((a) => a.teacher_profile_id === profileRow?.id);
