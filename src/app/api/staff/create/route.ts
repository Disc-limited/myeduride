...
export async function POST(request: NextRequest) {
  try {
    ...
    // Upsert the default 'staff' role and any additional roles to prevent conflict crashes
    const rolesToInsert = [
      { user_id: userId, school_id, role: 'staff', is_active: true }
    ];
    if (finalTeacherResponsibility === 'class_teacher' || finalTeacherResponsibility === 'subject_teacher' || finalTeacherResponsibility === 'both') {
      rolesToInsert.push({ user_id: userId, school_id, role: 'teacher', is_active: true });
    }
    if (accessRole !== 'staff' && accessRole !== 'teacher') {
      rolesToInsert.push({ user_id: userId, school_id, role: accessRole, is_active: true });
    }

    const { error: roleError } = await supabase
      .from('user_school_roles')
      .upsert(rolesToInsert, { onConflict: 'user_id,school_id,role' });
    ...
    if (STAFF_PROFILE_ACCESS_ROLES.has(accessRole)) {
      ...
      if (existingProfile?.id) {
        ...
        updatePayload.custom_role_id = custom_role_id || null;
        updatePayload.teacher_responsibility = finalTeacherResponsibility;
