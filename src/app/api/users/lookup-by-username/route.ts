...
import { usernameLookupCache } from '@/lib/auth/username-cache';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `${username}:${schoolId || 'global'}:${scope}`;
    const result = await usernameLookupCache.getOrFetch(cacheKey, () =>
      lookupUserByUsernameDetailed(supabase, username, {
        schoolId,
        scope,
      })
    );
