  ...
  try {
    ...
    const studentManifest = assignedStudents.map((st) => {
      ...
      return {
        ...
        is_not_ready_yet: trip?.readiness_status === 'not_ready' || Boolean(trip?.is_not_ready_yet),
        delayed_minutes: trip?.delayed_minutes || null,
        shifted_pickup_time: trip?.shifted_pickup_time || null,
        not_ready_reason: trip?.not_ready_reason || null,
        not_ready_at: trip?.not_ready_at || null,
