...
export async function GET(request: NextRequest) {
  try {
    ...
    const viewParam = sp.get('view') || 'all'; // 'all' | 'students' | 'staff'
    ...
    const includeStaff = caps.canStaffReports && viewParam !== 'students';
    const includeStudents = caps.canStudentReports && viewParam !== 'staff';
    ...
    if (includeStudents && caps.studentIds != null && caps.studentIds.length === 0) {
    ...
    if (studentIds.length === 0 && includeStudents) {
      ...
      if (includeStaff && reportType === 'daily') {
        ...
        emptyPayload.staff_summary = {
          ...
          late: staffOnly.filter((s) => s.status === 'late').length,
    ...
    if (studentIds.length === 0 && includeStaff && (!caps.canStudentReports || !includeStudents)) {
      ...
      if (reportType === 'daily') {
        ...
        return NextResponse.json({
          ...
          staff_summary: {
            ...
            late: staffReport.filter((s) => s.status === 'late').length,
