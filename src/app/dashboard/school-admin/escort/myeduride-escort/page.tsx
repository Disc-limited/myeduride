...
export default function MyEduRideEscortManagementPage() {
  ...
  return (
    <div className="space-y-6 font-sans text-slate-800 p-4 md:p-6 max-w-7xl mx-auto">
      ...
      {/* MODAL 1: ASSIGN STUDENT TO MYEDURIDE ESCORT */}
      {assignModalOpen && (
        <div className="fixed inset-0 bg-slate-950/75 z-50 flex items-center justify-center p-4 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl border border-slate-200 space-y-4 my-8">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-black text-slate-900 text-base">Assign Student to MyEduRide Escort</h3>
                  <p className="text-xs text-slate-500 font-medium">
                    Calculates road distance &amp; trip pricing, informs City Manager for clearance, and alerts parents immediately.
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleAssignStudentToEscort} className="space-y-4 text-xs">
              {/* STUDENT SELECTION */}
              <div>
                <label className="font-black text-slate-800 block mb-1">
                  1. Select Enrolled Student <span className="text-rose-500">*</span>
                </label>
                <select
                  value={selectedStudentId}
                  onChange={(e) => setSelectedStudentId(e.target.value)}
                  required
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl font-bold text-slate-800 focus:outline-none focus:border-emerald-500"
                >
                  {students.map((st: any) => (
                    <option key={st.id} value={st.id}>
                      {st.first_name} {st.last_name} ({st.class?.name || 'Class'} · {st.house_address || 'Home Pinned'})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                <label className="font-black text-slate-800 block mb-1">
                  2. Select Vetted MyEduRide Escort <span className="text-rose-500">*</span>
                </label>
                <select
                  value={selectedAssignEscortId}
                  onChange={(e) => setSelectedAssignEscortId(e.target.value)}
                  required
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-xl font-bold text-slate-800 focus:outline-none focus:border-emerald-500"
                >
                  {escorts.map((esc: any) => (
                    <option key={esc.id} value={esc.id}>
                      {esc.fullName || esc.name} — {esc.operatingArea} ({esc.vehicle?.regNumber || 'Vehicle'})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div>
                <label className="font-black text-slate-800 block mb-1">
                  3. Trip Schedule Selection <span className="text-rose-500">*</span>
                </label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Morning Pickup Time</label>
                  <input
                    type="time"
                    value={pickupTime}
                ...
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Afternoon Dropoff Time</label>
                  <input
                    type="time"
                    value={dropoffTime}
              ...
              {/* AUTOMATED DISTANCE & FARE CALCULATION PREVIEW BOX */}
              {livePreview && (
                <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-50 border border-emerald-200 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="font-black text-emerald-900 uppercase text-[10px] tracking-wider flex items-center gap-1.5">
                      <Sparkles size={13} className="text-emerald-600" />
                      Automatic Geolocation Distance &amp; Fare Engine
                    </span>
                    <span className="font-mono font-extrabold text-emerald-800 bg-white px-2 py-0.5 rounded-md border border-emerald-200 text-xs">
                      {livePreview.distanceKm} km
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-white/90 p-2 rounded-xl border border-emerald-100 shadow-2xs">
                      ...
                      <span className="font-black text-slate-900 text-sm">
                        ₦{livePreview.morningFare.toLocaleString()}
                      </span>
                    </div>
                    <div className="bg-white/90 p-2 rounded-xl border border-emerald-100 shadow-2xs">
                      <span className="text-[10px] text-slate-500 font-bold block">Afternoon Trip</span>
                      <span className="font-black text-slate-900 text-sm">
                        ₦{livePreview.afternoonFare.toLocaleString()}
                      </span>
                    </div>
                    <div className="bg-emerald-600 p-2 rounded-xl text-white shadow-xs">
                      ...
                      <span className="font-black text-white text-sm">
                        ₦{livePreview.dailyFare.toLocaleString()}
                      </span>
                    </div>
                  </div>

                  <div className="text-[11px] text-slate-600 space-y-0.5 pt-1 border-t border-emerald-200/60">
                    <p>🏫 <strong>Campus:</strong> {school?.name || 'School Main Campus'}</p>
                    <p>🏡 <strong>Doorstep:</strong> {livePreview.houseAddress}</p>
                  </div>

                  <div className="p-2 rounded-xl bg-emerald-100/60 text-emerald-900 text-[10px] font-medium flex items-center gap-1.5">
                    <CheckCircle2 size={13} className="shrink-0 text-emerald-700" />
                    <span>
                      The City Manager will be automatically informed for approval. Parents will receive this information instantly under their <strong>MyEduRide Escort Hub</strong>.
                    </span>
                  </div>
                </div>
              )}
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Route &amp; Safety Instructions (Optional)</label>
                <textarea
                  rows={2}
                  placeholder="e.g. Please pick up at the green gate. Handover requires confirmation PIN."
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setAssignModalOpen(false)}
                  className="px-4 py-2 rounded-xl border border-slate-200 font-bold text-slate-600 cursor-pointer hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
                ...
                >
                  <Send size={15} />
                  <span>{submittingAssign ? 'Assigning & Notifying...' : 'Assign Escort & Submit for CM Clearance'}</span>
                </button>
              </div>
            </form>
          </div>
