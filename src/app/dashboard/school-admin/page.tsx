...
export default function SchoolAdminDashboard() {
  const [stats, setStats] = useState({
    total_students: 0, total_teachers: 0, total_parents: 0, total_staff: 0,
    student_stats: { total: 0, present: 0, late: 0, absent: 0 },
    staff_stats: { total: 0, present: 0, late: 0, absent: 0 },
  });
  ...
  return (
    <div className="page-shell max-w-6xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 mb-8">
        ...
        <div className="dash-stat">
          <div>
            <p className="text-sm text-gray-500 mb-1">Students Present</p>
            <p className="text-3xl sm:text-4xl font-bold text-gray-900">
              {formatNumber((stats.student_stats?.present || 0) + (stats.student_stats?.late || 0))}
            </p>
          </div>
        ...
        <div className="dash-stat">
          <div>
            ...
            <p className="text-3xl sm:text-4xl font-bold text-gray-900">
              {formatNumber((stats.staff_stats?.present || 0) + (stats.staff_stats?.late || 0))}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <button 
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all min-h-[44px] min-w-[80px] ${attendanceView === 'students' ? 'bg-primary-100 text-primary-700' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}
              >
                Students
              </button>
              <button 
                onClick={() => setAttendanceView('staff')}
                className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all min-h-[44px] min-w-[80px] ${attendanceView === 'staff' ? 'bg-primary-100 text-primary-700' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}
              >
                Staff
              </button>
            </div>
            ...
            <div className="flex flex-col items-center">
              <div className="relative w-40 h-40">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                  {(() => {
                    const sStats = stats.student_stats || { present: 0, late: 0, absent: 0, total: 0 };
                    const total = sStats.present + sStats.late + sStats.absent || 1;
                    const onTimePercent = (sStats.present / total) * 100;
                    const latePercent = (sStats.late / total) * 100;
                    const absentPercent = (sStats.absent / total) * 100;
                    const radius = 40;
                    ...
                    return (
                      <>
                        ...
                        {sStats.present > 0 && <circle cx="50" cy="50" r={radius} fill="none" stroke="#10b981" strokeWidth="12" strokeDasharray={`${(onTimePercent / 100) * circumference} ${circumference}`} strokeDashoffset={`-${(onTimeOffset / 100) * circumference}`} />}
                        {sStats.late > 0 && <circle cx="50" cy="50" r={radius} fill="none" stroke="#f59e0b" strokeWidth="12" strokeDasharray={`${(latePercent / 100) * circumference} ${circumference}`} strokeDashoffset={`-${(lateOffset / 100) * circumference}`} />}
                        {sStats.absent > 0 && <circle cx="50" cy="50" r={radius} fill="none" stroke="#ef4444" strokeWidth="12" strokeDasharray={`${(absentPercent / 100) * circumference} ${circumference}`} strokeDashoffset={`-${(absentOffset / 100) * circumference}`} />}
                      </>
                    );
                  })()}
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <p className="text-2xl font-bold">{stats.student_stats?.total || 0}</p>
                  <p className="text-[10px] text-gray-500">Students</p>
                </div>
              </div>
              <div className="flex justify-center gap-4 mt-4 w-full">
                <span className="flex items-center gap-1.5 text-xs"><span className="w-2.5 h-2.5 rounded-full bg-green-500" /> On Time ({stats.student_stats?.present || 0})</span>
                <span className="flex items-center gap-1.5 text-xs"><span className="w-2.5 h-2.5 rounded-full bg-amber-500" /> Late ({stats.student_stats?.late || 0})</span>
                <span className="flex items-center gap-1.5 text-xs"><span className="w-2.5 h-2.5 rounded-full bg-red-500" /> Absent ({stats.student_stats?.absent || 0})</span>
              </div>
            </div>
          ) : (
            <div className="flex flex-col items-center">
              <div className="relative w-40 h-40">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                  {(() => {
                    const stStats = stats.staff_stats || { present: 0, late: 0, absent: 0, total: 0 };
                    const total = stStats.present + stStats.late + stStats.absent || 1;
                    const onTimePercent = (stStats.present / total) * 100;
                    const latePercent = (stStats.late / total) * 100;
                    const absentPercent = (stStats.absent / total) * 100;
                    const radius = 40;
                    const circumference = 2 * Math.PI * radius;
                    const onTimeOffset = 0;
                    const lateOffset = onTimePercent;
                    ...
                    return (
                      <>
                        <circle cx="50" cy="50" r={radius} fill="none" stroke="#e5e7eb" strokeWidth="12" />
                        {stStats.present > 0 && <circle cx="50" cy="50" r={radius} fill="none" stroke="#10b981" strokeWidth="12" strokeDasharray={`${(onTimePercent / 100) * circumference} ${circumference}`} strokeDashoffset={`-${(onTimeOffset / 100) * circumference}`} />}
                        {stStats.late > 0 && <circle cx="50" cy="50" r={radius} fill="none" stroke="#f59e0b" strokeWidth="12" strokeDasharray={`${(latePercent / 100) * circumference} ${circumference}`} strokeDashoffset={`-${(lateOffset / 100) * circumference}`} />}
                        {stStats.absent > 0 && <circle cx="50" cy="50" r={radius} fill="none" stroke="#ef4444" strokeWidth="12" strokeDasharray={`${(absentPercent / 100) * circumference} ${circumference}`} strokeDashoffset={`-${(absentOffset / 100) * circumference}`} />}
                      </>
                    );
                  })()}
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <p className="text-2xl font-bold">{stats.staff_stats?.total || 0}</p>
                  <p className="text-[10px] text-gray-500">Staff</p>
                </div>
              </div>
              <div className="flex justify-center gap-4 mt-4 w-full">
                <span className="flex items-center gap-1.5 text-xs"><span className="w-2.5 h-2.5 rounded-full bg-green-500" /> On Time ({stats.staff_stats?.present || 0})</span>
                <span className="flex items-center gap-1.5 text-xs"><span className="w-2.5 h-2.5 rounded-full bg-amber-500" /> Late ({stats.staff_stats?.late || 0})</span>
                <span className="flex items-center gap-1.5 text-xs"><span className="w-2.5 h-2.5 rounded-full bg-red-500" /> Absent ({stats.staff_stats?.absent || 0})</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="card lg:col-span-2">
          ...
          <div className="space-y-3">
            {filteredActivity.slice(0, 8).map((record: any) => {
              const isStaff = record.entity_type === 'staff';
              const nameParts = (record.name || 'Staff Member').split(' ');
              const fName = nameParts[0] || '';
              const lName = nameParts.slice(1).join(' ') || '';
              return (
                <div key={record.id} className="flex items-center gap-3">
                  <StudentAvatar
                    photoUrl={isStaff ? undefined : record.photo_url}
                    firstName={fName}
                    lastName={lName}
                    size="sm"
                    accentColor={isStaff ? "#3b82f6" : "#64748b"}
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{record.name}</p>
                    <p className="text-xs text-gray-400 flex items-center gap-1.5">
                      <span>{record.type === 'arrival' ? 'Arrived' : 'Left'}</span>
                      {isStaff && <span className="px-1.5 py-0.5 text-[9px] font-semibold bg-blue-100 text-blue-800 rounded">Staff</span>}
                    </p>
                  </div>
